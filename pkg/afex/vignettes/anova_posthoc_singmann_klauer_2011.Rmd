...
a1 # the default print method prints a data.frame produced by nice_anova 
```
...
The object printed per default for `afex_aov` objects (produced by `nice_anova`) can also be printed nicely using `knitr`:

```{r, results='asis', }
knitr::kable(nice_anova(a1))
```
