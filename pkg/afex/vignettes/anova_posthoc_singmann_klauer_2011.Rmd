...
To get the full ANOVA table for the model, we simply pass it to `ez.glm` (`aov.car` or `aov4` would be alternatives producing the same results) using the design as described above. We save the returned object for further analysis.

```{r}
a1 <- ez.glm("id", "response", sk2011.1, between = "instruction", 
       within = c("inference", "plausibility"))
a1 # the default print method prints a data.frame produced by nice.anova 
```
...
The object printed per default for `afex_aov` objects (produced by `nice.anova`) can also be printed nicely using `knitr`:

```{r, results='asis', }
knitr::kable(nice.anova(a1))
```
