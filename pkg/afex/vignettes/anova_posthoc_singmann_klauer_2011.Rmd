...
require(afex) # needed for ANOVA
require(lsmeans) # for post-hoc tests and plotting
require(multcomp) # for elaborated multiple comparison procedures to control for Type 1 errors.
...
To get the full ANOVA table for the model, we simply pass it to `ez.glm` (`aov.car` or `aov4` would be alternatives producing the same results) using the design as described above.

```{r}
ez.glm("id", "response", sk2011.1, between = "instruction", 
...
To further analyze the data we need to pass it to package `lsmeans`, a package that offers great functionality for both plotting and contrasts of all kind. A lot of information on `lsmeans` can be obtained in [its vignette][http://cran.r-project.org/web/packages/lsmeans/vignettes/using-lsmeans.pdf]. While `lsmeans` can work with many type of objects, it unfortunately cannot work with objects of class `Anova` or `Anova.mlm`, which are the objects returned from `car::Anova`, the function used by `afex` to produce the ANOVA results table. However, it can work with objects as returned from `aov`. Consequently, `afex` now contains the possibility to obtain a model fitted with `aov` for passing it to `lsmeans`, by setting `return="aov"`. Note however, that for obtaining the test on effects (i.e., the ANOVA table) the default output produces more sensible results. 


```{r}
a1 <- ez.glm("id", "response", sk2011.1, between = "instruction", 
       within = c("inference", "plausibility"), 
       return = "aov")
```
...
Objects returned from `lsmeans` can also be used to test specific contrasts. For this, we can simply create a list, where each element correspond to a contrasts. A contrast is defined as a vector on the reference grid (i.e., the object returned from `lsmeans`, here `m3`). For example, we might be interested in whether there is a difference between the valid and invalid inferences in each of the two conditions.
...
Function `lsmip` from package `lsmeans` can be used for plotting the data. As said initially, we are interested in the three-way interaction of instruction with inference, plausibility, and instruction. A plot of this interaction could be the following:
...
```{r fig.width=7.5, fig.height=4}
a2 <- ez.glm("id", "response", sk2011.1, between = "instruction", 
...
* While the df are Greenhouse-Geisser corrected per default for within-subject factors with more than two levels in the returned ANOVA table (i.e., the default output of `ez.glm` et al.), this is not the case for the post-hoc tests. The contrasts use uncorrected degrees of freedom that Satterthwaite approximated. This most likely produces anti-conservative tests is compound symmetry is violated.
* For unbalanced samples, `aov` is usually not the correct choise. This is why the default output is produced by `car::Anova`. However, for `lsmeans` we need to use `aov` models. It therefore offers the option to weight the marginal means differently depending on different group sizes, For example such that each group is assumed to be of equal size (i.e., `weights = "equal"`) or proportionally (i.e., `weights = "proportional"`). See help of `lsmeans` for more information.
* Choosing the right correction Type 1 error correction can be difficult. In fact `multcomp` comes with an accompanying book (Bretz, Hothorn, & Westfall, 2011). If the degrees-of-freedom of all contrasts are identical using `multcomp`'s method `free` is more powerful than simply using the Bonferroni-Holm method. `free` is a generalization of the Bonferroni-Holm method that takes the correlations among the model parameters into account and uniformly more powerful.
* For data sets with many within-subject factors, creating the `aov` object can take some time (i.e., compared to producing the ANOVA table which is usually very fast). Those objects are also comparatively large, often multiple MB.

# References 

* Bretz, F., Hothorn, T., & Westfall, P. H. (2011). _Multiple comparisons using R_. Boca Raton, FL: CRC Press.
