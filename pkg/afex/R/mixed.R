...
#' @param check.contrasts \code{logical}. Should contrasts be checked and (if necessary) changed to \code{"contr.sum"}? See Details. The default (\code{"TRUE"}) is taken from \code{\link{afex.options}}.
...
mixed <- function(formula, data, type = afex.options("type"), method = afex.options("method.mixed"), per.parameter = NULL, args.test = list(), test.intercept = FALSE, check.contrasts = afex.options("check.contrasts"), set.data.arg = TRUE, progress = TRUE, cl = NULL, ...) {
  if (check.contrasts) {
    #browser()
